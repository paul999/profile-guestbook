...
class ucp_profile_guestbook
{
	...
	function main($id, $mode)
	{
		...
		if ($submit)
		{
			...
			if (!$config['jab_enable'] && in_array($data['notifymethod'], array(GB_NOTIFY_IM, GB_NOTIFY_IM_PM, GB_NOTIFY_ALL)))
			{
				$error[] = 'GB_JABBER_DISABLED';
			}
			
			if (!$config['email_enable'] && in_array($data['notifymethod'], array(GB_NOTIFY_EMAIL, GB_NOTIFY_EMAIL_PM, GB_NOTIFY_ALL)))
			{
